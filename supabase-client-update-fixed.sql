-- Step 1: Create clients table first (ALREADY COMPLETED ✅)
...
-- Step 2: Add columns to existing tables (one at a time to avoid conflicts)
...
ALTER TABLE snippets 
ADD CONSTRAINT IF NOT EXISTS fk_snippets_client_id 
...
ALTER TABLE generation_sessions 
ADD CONSTRAINT IF NOT EXISTS fk_generation_sessions_client_id 
FOREIGN KEY (client_id) REFERENCES clients(id) ON DELETE CASCADE;
...
ALTER TABLE composed_prompts 
ADD CONSTRAINT IF NOT EXISTS fk_composed_prompts_client_id 
FOREIGN KEY (client_id) REFERENCES clients(id) ON DELETE CASCADE;

-- Step 6: Create indexes (with IF NOT EXISTS)
...
-- Step 10: Create or replace the function
